...
//  var trianglesCount: Int = 0 // in superclass
  ...
  func makeBuffers(vertices: [Float32], triangles: [UInt16]) {
    verticesCount = vertices.count / 6
    trianglesCount = triangles.count / 3
//    print("make bugffers vertices ")
    ...
    for i in 0..<verticesCount {
      verticesPointer[i * verticesStride + 0] = vertices[i * verticesStride + 0] // first point
      verticesPointer[i * verticesStride + 1] = vertices[i * verticesStride + 1]
      verticesPointer[i * verticesStride + 2] = vertices[i * verticesStride + 2]
      verticesPointer[i * verticesStride + 3] = vertices[i * verticesStride + 3] // second point
      verticesPointer[i * verticesStride + 4] = vertices[i * verticesStride + 4]
      verticesPointer[i * verticesStride + 5] = vertices[i * verticesStride + 5]
